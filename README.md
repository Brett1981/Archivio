...
The review workspace separates plans that are already organised from those needing metadata review or conflict resolution. A reviewer can compare every original filename, library path, embedded title, author, album, track, duration, and available online cover beside the correction controls, and can open a selected source in the Windows default audio player. Clean 100%-confidence plans become eligible for bulk approval without individual review, including multipart books when Metaroq verifies a complete unique order, consistent identity, and conflict-free destination; the author/title fields are corrections, not mandatory confirmations. Missing parts, duplicate ordering, mixed identities, unresolved metadata, and destination conflicts still require review. Durable corrections immediately regenerate and revalidate the plan without changing any media files. The automatic Metaroq identity or genre suggestion can be restored at any time.
